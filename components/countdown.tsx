...
function TimeBox({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="grid h-12 w-full place-items-center rounded-lg bg-white text-xl font-black tabular-nums text-primary shadow-sm sm:h-16 sm:text-3xl">
        {value}
      </div>
...
export function Countdown({ className }: { className?: string }) {
  ...
  if (now === null) {
    ...
    return (
      <div
        className={cn(
          "rounded-xl bg-primary px-3 py-4 text-primary-foreground sm:px-6",
          className
        )}
        aria-hidden="true"
      >
